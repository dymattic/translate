...
class TranslatorBot(Plugin):
    ...
    async def subscribe(self, evt: MessageEvent, source_lang: list, target_lang: list, provider: str) -> None:
        ...
        if not room_id: 
            self.db.create_autotranslate(room_id=evt.room_id, user_id=evt.sender, source_lang=source_lang, target_lang=target_lang, provider=provider)
        else:
            self.db.update_autotranslate(room_id=evt.room_id, user_id=evt.sender, source_lang=source_lang, target_lang=target_lang, provider=provider)
    ...
    async def event_handler(self, evt: MessageEvent) -> None:
        if (langdetect is None or evt.content.msgtype == MessageType.NOTICE
                or evt.sender == self.client.mxid or evt.content.body[0:3] == '!tr'):
            return
        atc = self.db.get_autotranslate_by_room(evt.room_id)
        if atc:
        ...
        if self.is_acceptable_soft(detected_lang, accepted_languages):
            ...
            if self.is_acceptable(result.source_language, accepted_languages):
                ...
                for atc_main_language in main_language[1:]:
                    try:
                        result = await self.translator.translate(evt.content.body, to_lang=atc_main_language, from_lang=from_lang)
                    except:
                        await evt.respond(f"[{evt.sender}](https://matrix.to/#/{evt.sender}) "
                                          f"Provider __{self.config['provider']['id']}__ not reachable!!")
                        return
                ...
                for atc_main_language in main_language:
                    for atc_accepted_language in accepted_languages:
                        if atc_main_language != atc_accepted_language:
                            try:
                                result = await self.translator.translate(evt.content.body, to_lang=atc_main_language, from_lang=atc_accepted_language)
                            except:
                                await evt.respond(f"[{evt.sender}](https://matrix.to/#/{evt.sender}) "
                                                  f"Provider __{self.config['provider']['id']}__ not reachable!!")
                                return
...
"""
        ...
        if auto == 'setauto' and language:
            await self.subscribe(evt=evt, source_lang=language[0], target_lang=language[1], provider = self.config["provider"]["id"])
            return
        if auto == 'unsetauto':
            await self.unsubscribe(evt=evt)
            return
